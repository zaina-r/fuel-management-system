  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data && Array.isArray(response.data.vehiclesDtoList)) {
          setVehicles(response.data.vehiclesDtoList);
          
          // Calculate total fuel
          const fuelSum = response.data.vehiclesDtoList.reduce(
            (sum, vehicle) => sum + (parseFloat(vehicle.availableFuelCapacity) || 0),
            0
          );
          setTotalFuel(fuelSum);
          
          // Calculate total transactions (mock data - replace with actual API call)
          setTotalTransactions(response.data.vehiclesDtoList.length * 3); 
        } else {
          setVehicles([]);
        ...
        setError(error.response?.data?.message || "Failed to fetch vehicle data");
      } finally {
        setLoading(false);
      }
    };
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
              ...
              <div className="bg-gray-900 rounded-lg p-4 shadow border border-gray-700">
                ...
                <p className="text-white text-lg font-semibold">
                  {["Bike", "Bus", "Van"].filter(hasType).join(", ") || "None"}
                </p>
              </div>
              <div className="bg-gray-900 rounded-lg p-4 shadow border border-gray-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                ...
                {vehicleTypeData.length > 0 ? (
                  ...
                  <div className="text-center text-gray-400 py-10">No vehicle type data available</div>
                )}
              </div>
              ...
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                ...
                {fuelTypeData.length > 0 ? (
                  ...
                  <div className="text-center text-gray-400 py-10">No fuel type data available</div>
                )}
              </div>
            </div> 
            ...
            <div className="text-gray-400 py-6 flex items-center gap-8">
              <div className="flex items-center gap-2 border p-1 rounded-lg border-yellow-600 text-sm">
                <BadgeCent className="inline w-[20px] rounded-full text-orange-600" /> 
                Total Transaction{" "}
                <span className="text-white text-xl">{totalTransactions}</span>
              </div>
              <div className="flex items-center gap-2 p-1 border rounded-lg border-yellow-600 text-sm">
                <Fuel className="inline w-[20px] rounded-full text-orange-600" /> 
                Available Fuel{" "}
                <span className="text-white text-xl">{totalFuel.toFixed(1)}L</span>
              </div>
            </div>
            ...
            <table className="w-full text-white text-sm border border-gray-700">
              <thead className="bg-gray-800">
                <tr>
                  ...
                  <th className="p-2 border border-gray-700">Fuel Capacity (L)</th>
                </tr>
              </thead>
              <tbody>
                {filteredVehicles.length > 0 ? (
                  filteredVehicles.map((vehicle) => (
                    <tr key={vehicle.id} className="bg-gray-900 hover:bg-gray-800">
                      ...
                      <td className="p-2 border border-gray-700">
                        {parseFloat(vehicle.availableFuelCapacity).toFixed(1)}
                      </td>
                    </tr>
                  ))
                ) : (
